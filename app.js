...
function initPitwall() {
  ...
  function generateAiSetupAndSave(data) {
    ...
    if ((c.includes('valkyrie') || p.includes('valkyrie')) && (t.includes('monza') || p.includes('monza') || p.includes('lmu'))) {
      ...
      // Valkyrie HY setup - generated by Vistula AI Engine
      const svmContent =
        '//[[gMa1.002f (c)2016    ]] [[            ]]\n' +
        '// Vistula AI Engine - Aston Martin Valkyrie HY Setup\n' +
        'VehicleClassSetting="Hypercar AstonMartin_Valkyrie_HY"\n' +
        '\n[GENERAL]\n' +
        ...
        '\n[SUSPENSION]\n' +
        'FrontAntiSwaySetting=0//Detached\nRearAntiSwaySetting=3//P4\n' +
      ...
      // Keyword matching - handles English and Polish
      if (p.includes('understeer') || p.includes('podsterown') || p.includes('podsterow') || p.includes('wchodz') || p.includes('wejscie') || p.includes('nie skr')) {
        solution += 'Zmniejsz przedni stabilizator (ARB) o 1-2 kliki. Zwieksz Rake platformy podnoszac tyl o 2mm. Rozwaz wiekszy Toe-Out z przodu dla lepszej reakcji na wejsciu. ';
      }
      if (p.includes('oversteer') || p.includes('nadsterown') || p.includes('nadsterow') || p.includes('tyl ucieka') || p.includes('snap') || p.includes('tyl sie')) {
        solution += 'Zwieksz tylne skrzydlo o 1 stopien. Zmniejsz tylny ARB o 1-2 kliki. Utwardz sprezyny przednie o 1 klik, by odciazyc tyl na wejsciu. Zredukuj Diff Power do 40-42% dla wiekszej stabilnosci. ';
      }
      if (p.includes('opon') || p.includes('tyre') || p.includes('tire') || p.includes('temperatur') || p.includes('temp') || p.includes('psi') || p.includes('kpa') || p.includes('cisnien') || p.includes('camber')) {
        solution += 'Dane termalne odchylone poza okno MoTeC. Obniz cisnienie o ~0.4 PSI z goracej strony osi. Zweryfikuj Camber (jesli Inner gorący, zwieksz negatywny o 0.2-0.3 deg). Optimum: 85-95C. ';
      }
      if (p.includes('bump') || p.includes('kraweznik') || p.includes('nierownos') || p.includes('trakcj') || p.includes('rebound') || p.includes('amortyz') || p.includes('damper')) {
        solution += 'Fast Bump na tylnej osi zbyt sztywny. Zmniejsz o 2-3 kliki. Sprawdz Damper Histogramy w MoTeC – os Y powinna miescic sie w +/-0.15 m/s przy normalnej jezdzie. ';
      }
      if (p.includes('hamul') || p.includes('brake') || p.includes('blokuj') || p.includes('abs') || p.includes('bias') || p.includes('migration') || p.includes('lockup')) {
        solution += 'Problem z hamulcami: skoryguj Brake Bias o 0.5-1% do tylu (np. 56% na 55.5%). Zastosuj Brake Migration +0.5%F. Sprawdz temperature tarcz (optimum: 400-600C). ';
      }
      if (p.includes('fuel') || p.includes('paliw') || p.includes('bak') || p.includes('consumption') || p.includes('zuzycie') || p.includes('hybrid') || p.includes('energy')) {
        solution += 'Strategia paliwa: Fuel Map na Economy w Low-Speed, Rich na dlugich prostych. Hypercar: regeneracja w strefach hamowania oszczedza ~0.3L/lap. ';
      }
      if (solution === '') {
        solution = 'Analiza ogolna: zacznij od Ride Height do minimum mechanicznego, nastepnie ustaw Damper Histogramy w MoTeC. Sprawdz balans osi: Slip Angle przod vs tyl w proporcji 1:1.1 dla neutralnej reakcji. ';
      }
    }
